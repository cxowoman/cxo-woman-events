...
const state = {
  ...
  cloudProposalsLoaded: !cloudIsConfigured(),
};
...
async function fetchWithTimeout(url, options = {}, timeoutMs = 20000) {
  ...
  try {
    ...
    if (error?.name === "AbortError") {
      throw new Error("同步等待太久，請稍後再按一次。");
    }
    throw error;
  } finally {
...
function isActiveAdminProposal(proposal) {
  return proposal.status === "pending" || isOpenForRegistration(proposal);
}

function isHistoricalProposal(proposal) {
  return proposal.status === "rejected" || eventHasEnded(proposal);
}
...
function findDuplicateRegistration(proposalId, email, phone) {
  const emailKey = normalizedEmail(email);
  const phoneKey = normalizedPhone(phone);
  return state.data.registrations.find((entry) => {
    if (entry.proposalId !== proposalId) return false;
    return (
      (emailKey && normalizedEmail(entry.email) === emailKey) ||
      (phoneKey && normalizedPhone(entry.phone) === phoneKey)
    );
  });
}
...
async function syncCloudProposals(includeAll = false) {
  ...
  state.cloudProposalsLoaded = true;
  saveData();
}
...
function renderRegistrationDrivePanel() {
  ...
  panel.innerHTML = `
    <div>
      ...
      <h2>雲端報名資料</h2>
      <p>
        夥伴可透過 Google Sheet 查看或下載報名資料；按「同步 Google Sheet」會把後台缺漏的報名補進總表。
      </p>
    </div>
...
async function syncGoogleSheet() {
  if (!cloudIsConfigured()) {
    throw new Error("尚未連接 Supabase，無法同步 Google Sheet。");
  }

  const accessToken = sessionStorage.getItem(ACCESS_TOKEN_KEY);
  if (!accessToken) {
    throw new Error("請先登入管理員帳號，再同步 Google Sheet。");
  }

  const spreadsheetId = spreadsheetIdFromUrl(state.data.settings.registrationDriveUrl);
  if (!spreadsheetId) {
    throw new Error("請先到網站設定貼上 Google Sheet 總表連結。");
  }

  const response = await fetchWithTimeout(
    `${CLOUD_CONFIG.supabaseUrl}/functions/v1/sync-google-sheet`,
    {
      method: "POST",
      headers: cloudHeaders(accessToken),
      body: JSON.stringify({ spreadsheetId, sheetName: "報名資料" }),
    },
  );
  const result = await response.json().catch(() => ({}));

  if (!response.ok) {
    if (/insufficient authentication scopes/i.test(result.error || "")) {
      throw new Error("Google Sheet 尚未授權寫入，請先補 Google Sheets 權限。");
    }
...
function renderProposalList() {
  ...
  const proposals = state.data.proposals
    .filter(
      (proposal) =>
        filter === "all" ||
        (filter === "active" && isActiveAdminProposal(proposal)) ||
        (filter === "history" && isHistoricalProposal(proposal)) ||
        ...
        ${
          entries.length
            ? entries
                ...
                .join("")
            : `<div class="empty-state"><h3>此活動目前沒有報名</h3><p>這裡只顯示目前選取活動的報名；完整名單請看上方「會員報名總表」。</p></div>`
        }
      </div>
    </div>
  `;
...
function renderRegistrationPage(proposalId) {
  ...
  if (cloudIsConfigured() && !state.cloudProposalsLoaded) {
    shell.innerHTML = `
      <div class="empty-state">
        <h3>正在載入雲端活動資料</h3>
        <p>請稍候，系統正在確認這個活動是否仍開放報名。</p>
      </div>
    `;
    return;
  }
...
async function handleRegistrationSubmit(event) {
  ...
  const memberPhone = formData.get("phone").trim();
  const duplicateRegistration = findDuplicateRegistration(
    proposal.id,
    memberEmail,
    memberPhone,
  );
  if (duplicateRegistration) {
    showToast("這個 Email 或手機已經報名過此活動，不需要重複送出。");
    return;
  }
...
$("#refreshCloudButton").addEventListener("click", async () => {
  ...
  const button = $("#refreshCloudButton");
  ...
  try {
    ...
    await Promise.all([syncCloudProposals(true), syncCloudRegistrations()]);
    render();
    showToast(`Google Sheet 已同步，新增 ${result.inserted || 0} 筆。`);
  } catch (error) {
    showToast(error.message || "更新失敗。");
  } finally {
    const currentButton = $("#refreshCloudButton");
    if (currentButton) {
...
if (cloudIsConfigured()) {
  const shouldSyncAdminData =
    isAdminAuthenticated() && Boolean(sessionStorage.getItem(ACCESS_TOKEN_KEY));
  Promise.all([
    syncCloudSettings(),
    syncCloudProposals(shouldSyncAdminData),
    shouldSyncAdminData ? syncCloudRegistrations() : Promise.resolve(),
  ])
    .then(render)
